...
# To compute results of EAI use following methods:
...
#     # ---- Error-Aware Intersection (EAI) ----
#     pos_ratio = gt_flat.mean()  
#     if pos_ratio == 0:
#         alpha, beta = 1.0, 1.0
#     else:
#         alpha = 1.0
#         beta = max(1.0, (1.0 / pos_ratio)) 

#     denom = tp + alpha * fp + beta * fn
#     eai = (tp / denom) if denom > 0 else 0.0

#     return {'tp': tp, 'fp': fp, 'fn': fn, 'precision': precision, 'recall': recall, 'f1': f1, 'eai': eai}


# def mean(list_of_metric_dicts):
#     if not list_of_metric_dicts:
#         return {'precision': 0.0, 'recall': 0.0, 'f1': 0.0, 'eai': 0.0}
#     precision = float(np.mean([m['precision'] for m in list_of_metric_dicts]))
#     recall = float(np.mean([m['recall'] for m in list_of_metric_dicts]))
#     f1 = float(np.mean([m['f1'] for m in list_of_metric_dicts]))
#     eai = float(np.mean([m['eai'] for m in list_of_metric_dicts]))
#     return {'precision': precision, 'recall': recall, 'f1': f1, 'eai': eai}
